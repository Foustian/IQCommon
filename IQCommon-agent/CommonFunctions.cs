                ...
                using (var conn = new SqlConnection(ConnString))
                {
                    ...
                    using (var cmd = conn.GetCommand("usp_common_IQ_MediaTypes_SelectWithRoles", CommandType.StoredProcedure))
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            {
                                Newtonsoft.Json.Linq.JObject jObj = Newtonsoft.Json.Linq.JObject.Parse(dashboardJsonData);

                                //List<DataChartTypes> chartTypes = (jObj["ChartTypes"]).Select(ct => (DataChartTypes)Enum.Parse(typeof(DataChartTypes), Convert.ToString(ct))).ToList();
                                List<DataChart> chartTypes = (jObj["ChartTypes"]).Select(ct => new DataChart { Title = Convert.ToString(ct["Title"]), DataChartType = (DataChartTypes)Enum.Parse(typeof(DataChartTypes), Convert.ToString(ct["DataChartType"])) }).ToList();

                                List<DataList> dataLists = (jObj["DataLists"]).Select(ct => new DataList { DataType = Convert.ToString(ct["DataType"]), ListType = (ListTypes)Enum.Parse(typeof(ListTypes), Convert.ToString(ct["ListType"])), TemplateType = (TemplateTypes)Enum.Parse(typeof(TemplateTypes), Convert.ToString(ct["TemplateType"])), Title = Convert.ToString(ct["Title"]), TitleColumn = Convert.ToString(ct["TitleColumn"]) }).ToList();

                                mediaType.ChartTypes = chartTypes;
                                mediaType.DataLists = dataLists;
                                mediaType.UseCanadaMap = Convert.ToBoolean(jObj["UseCanadaMap"]);
                                mediaType.CheckCanadaSettings = Convert.ToBoolean(jObj["CheckCanadaSettings"]);
                            }
