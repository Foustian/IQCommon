...
using IQSearch.Sentiment.Logic;
...
namespace IQSearch
{
    ...
    public class SearchEngine
    {
        ...
        public SearchTVResult SearchTV(SearchTVRequest request, Int32? timeOutPeriod = null, Boolean isSentimentCall = false, string CustomSolrFl = "")
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            SearchTVResult res = new SearchTVResult();

            try
            {
                IQLogger.Debug("IQSearch.SearchTV - Begin");
                ...
                // if StartDate or EndDate is passed then make search on gmtdatetime_dt.
                if (request.StartDate.HasValue || request.EndDate.HasValue)
                {
                    ...
                    FQuery.Append("gmtdatetime_dt:[")
                        .Append(request.StartDate.HasValue ? request.StartDate.Value.ToString("s", System.Globalization.CultureInfo.CurrentCulture) + "Z" : "*")
                        .Append(" TO ")
                        .Append(request.EndDate.HasValue ? request.EndDate.Value.ToString("s", System.Globalization.CultureInfo.CurrentCulture) + "Z" : "*")
                        .Append("]");
                }

                // if RLStationStartDate or RLStationEndDate is passed then lets make search on datetime_dt.
                if (request.RLStationStartDate.HasValue || request.RLStationEndDate.HasValue)
                {
                    if (FQuery.Length > 0)
                    {
                        FQuery.Append(" AND ");
                    }
                    FQuery.Append("datetime_dt:[")
                        .Append(request.RLStationStartDate.HasValue ? request.RLStationStartDate.Value.ToString("s", System.Globalization.CultureInfo.CurrentCulture) + "Z" : "*")
                        .Append(" TO ")
                        .Append(request.RLStationEndDate.HasValue ? request.RLStationEndDate.Value.ToString("s", System.Globalization.CultureInfo.CurrentCulture) + "Z" : "*")
                        .Append("]");
                }

                // if both ClipStartDate and ClipEndDate is passed then lets make search on date.
                if (request.ClipStartDate.HasValue || request.ClipEndDate.HasValue)
                {
                    if (FQuery.Length > 0)
                    {
                        FQuery.Append(" AND ");
                    ...
                    FQuery.Append("date:[")
                        .Append(request.ClipStartDate.HasValue ? request.ClipStartDate.Value.ToString("yyyy-MM-dd", System.Globalization.CultureInfo.CurrentCulture) + "Z" : "*")
                        .Append(" TO ")
                        .Append(request.ClipEndDate.HasValue ? request.ClipEndDate.Value.ToString("yyyy-MM-dd", System.Globalization.CultureInfo.CurrentCulture) + "Z" : "*")
                        .Append("]");
                }

                // if station(s) are passed , then lets search Stations
                if (request.Stations != null && request.Stations.Count > 0)
                {
                    IEnumerable<string> stations;
                    if (request.ExcludeStations != null && request.ExcludeStations.Count > 0)
                    {
                        stations = request.Stations.Except(request.ExcludeStations);
                    }
                    else
                    {
                    ...
                    {
                        FQuery = FQuery.Append("stationid:(");
                    }
                    else
                    {
                        FQuery = FQuery.Append(" AND stationid:(");
                    }

                    //bool IsFirstStation = true;

                    foreach (string _Station in stations)
                    {
                        FQuery.Append(' ').Append(_Station);
                        /*if (IsFirstStation)
                        {
                ...
                else if (request.ExcludeStations != null && request.ExcludeStations.Count > 0)
                {
                    if (string.IsNullOrEmpty(FQuery.ToString()))
                    {
                        FQuery = FQuery.Append("-stationid:(");
                    ...
                    foreach (string _Station in request.ExcludeStations)
                    {
                        FQuery.Append(' ').Append(_Station);
                    }
                    FQuery = FQuery.Append(" )");
                ...
                // if ExcludeIQDmaName(s) are passed, then lets exclude them
                if (request.ExcludeIQDmaName != null && request.ExcludeIQDmaName.Count() > 0)
                {
                    if (!string.IsNullOrEmpty(FQuery.ToString()))
                    {
                        FQuery = FQuery.Append(" AND");
                    }
                    FQuery = FQuery.Append(" NOT market:(");

                    foreach (string _IQ_Dma_Name in request.ExcludeIQDmaName)
                    {
                        FQuery = FQuery.AppendFormat("\"{0}\" ", _IQ_Dma_Name);
                    }
                ...
                if (request.LastSequenceId.HasValue && request.LastSequenceId.Value > 0)
                {
                    if (FQuery.Length > 0)
                    {
                        FQuery.Append(" AND ");
                    ...
                    FQuery.AppendFormat("seqid:[{0} TO *]", request.LastSequenceId.Value + 1);
                }
                ...
                IQLogger.BuildDebug("IQSearch.SearchTV() - Create Solr Request")
                    .SetValue("q", q)
                    .SetValue("fq", fq)
                    .SetValue("outRequest", request.IsOutRequest)
                    .Submit();
                ...
                IQLogger.BuildDebug("IQSearch.SearchTV() - Response Received").SetValue("timeElapsed", sw.Elapsed).Submit();
                ...
                if (!isSentimentCall)
                {
                    IQLogger.BuildDebug("IQSearch.SearchTV() - Parse Response").SetValue("timeElapsed", sw.Elapsed).Submit();
                    parseResponse(xDoc, res, Fl, FlTitle120);
                }
                else
                {
                    IQLogger.BuildDebug("IQSearch.SearchTV() - Parse Sentiment").SetValue("timeElapsed", sw.Elapsed).Submit();
                ...
                {
                    //CommonFunction.LogInfo("Running sentiment request.", request.IsPmgLogging, request.PmgLogFileLocation);
                    IQLogger.BuildDebug("IQSearch.SearchTV() - Running sentiment request.").SetValue("timeElapsed", sw.Elapsed).Submit();
                    makeSentimentRequest(res, timeOutPeriod);
                }

                sw.Stop();

                IQLogger.BuildDebug("IQSearch.SearchTV() - Complete")
                    .SetValue("totalResults", res.TotalHitCount)
                    .SetValue("timeElapsed", sw.Elapsed)
                    .Submit();
        ...
        public SearchTVResult SearchTVChart(SearchTVRequest request, out Boolean isError, Int32? timeOutPeriod = null)
        {
        ...
        public TVFullResult SearchFullTV(TVFullRequest request, Int32? timeOutPeriod = null, Boolean isSentimentCall = false, string CustomSolrFl = "", bool getFacets = false)
        {
            ...
            try
            {
                ...
                if (getFacets)
                {
                    Dictionary<string, List<string>> facets = GetTVFullFacets(new List<KeyValuePair<string, string>>(vars), request, sw);
        ...
        private Dictionary<string, List<string>> GetTVFullFacets(List<KeyValuePair<string, string>> vars, TVFullRequest request, Stopwatch sw)
        {
            var lstTasks = new List<Task>();
            var dictResults = new Dictionary<string, List<string>>();
            ...
            List<KeyValuePair<string, string>> companyVars = new List<KeyValuePair<string, string>>(vars);
            companyVars.Add(new KeyValuePair<string, string>("facet.field", "companiespaid"));
            companyVars.Add(new KeyValuePair<string, string>("facet.field", "companiesearned"));
            lstTasks.Add(Task.Factory.StartNew((object obj) => ExecuteFacetSearch(companyVars, "Company Facet Call", request.IsPmgLogging, request.PmgLogFileLocation), "CompanyFacet", TaskCreationOptions.AttachedToParent));
            ...
            foreach (KeyValuePair<string, SearchTVFullResult> kvResponse in dictResponses)
            {
                ...
                foreach (XmlNode node in facetNodes)
                {
                    ...
                    {
                        results.Add(cn.Attributes["name"].Value);
                    }
        ...
        private static void parseResponse(XmlDocument doc, SearchTVResult res, string highlightCCFeildName, string highlightTitleFeildName)
        {
            ...
            foreach (XmlNode hitNode in hitNodes)
            {
                ...
                int firstOffset = hit.TermOccurrences.Count > 0 ? hit.TermOccurrences[0].TimeOffset / 60 : 0;
                int firstIndex = Math.Max(hit.ListOfIQStartMinute.Count(m => m <= firstOffset) - 1, 0);
                if (ListOfTitle120.Count > 0)
                {
                    hit.FirstTitle120 = ListOfTitle120[firstIndex];
                }
                if (hit.ListOfIQClass.Count > 0)
                {
                    hit.FirstIQClass = hit.ListOfIQClass[firstIndex];
                }

                if (res.OriginalRequest.GenerateQuarterHour)
                {
                    hit.Quarters = genereateQuarterHits(hit, ListOfTitle120);
                }
        ...
        private static IDictionary<short, QuarterHit> genereateQuarterHits(Hit hit, List<string> ListOfTitle120)
        {
            var quarters = new SortedDictionary<short, QuarterHit>();
            foreach (var occurance in hit.TermOccurrences)
            {
                short startPoint = Convert.ToInt16(occurance.TimeOffset / 900 + 1);
                QuarterHit quarter;
                if (!quarters.TryGetValue(startPoint, out quarter))
                {
                    quarter = new QuarterHit(startPoint);
                    int firstOffset = occurance.TimeOffset / 60;
                    int firstIndex = Math.Max(hit.ListOfIQStartMinute.Count(m => m <= firstOffset) - 1, 0);
                    if (ListOfTitle120.Count > 0)
                    {
                        quarter.FirstTitle120 = ListOfTitle120[firstIndex];
                    }
                    if (hit.ListOfIQClass.Count > 0)
                    {
                        quarter.FirstIQClass = hit.ListOfIQClass[firstIndex];
                    }
                    quarters.Add(startPoint, quarter);
                }
                quarter.TermOccurrences.Add(occurance);
                quarter.TotalNoOfOccurrence++;
            }
            return quarters;
        }

        private void makeSentimentRequest(SearchTVResult res, Int32? timeOutPeriod = null)
        {
            if (res.Hits.Count > 0)
            {
                SearchTVRequest _SearchRequest = new SearchTVRequest();
                _SearchRequest.IsSentiment = false;
                _SearchRequest.ClientGuid = res.OriginalRequest.ClientGuid;
                ...
                _SearchRequest.IsPmgLogging = res.OriginalRequest.IsPmgLogging;
                _SearchRequest.PmgLogFileLocation = res.OriginalRequest.PmgLogFileLocation;
                SearchTVResult _res = SearchTV(_SearchRequest, timeOutPeriod, true);

                SentimentLogic logic = new SentimentLogic();
                foreach (Hit hit in res.Hits)
                {
                    //IQLogger.BuildDebug("Try getting Sentiment.").SetValue("IQCCKey", hit.Iqcckey).SetValue("searchRequest", res.OriginalRequest.Terms).Submit();
                    hit.Sentiments = _res.Hits.Where(h => h.Iqcckey.Equals(hit.Iqcckey)).Select(h => h.Sentiments).FirstOrDefault();
                    if (hit.Sentiments != null)
                    {
                        // If quarter hits have been generated, split the hour sentiments into quarters,
                        // and map the quarter sentiment objects to the quarter hits.
                        if (res.OriginalRequest.GenerateQuarterHour && hit.Quarters != null && hit.Quarters.Count > 0)
                        {
                            var quarterSentiments = logic.GetQuarterHourSentiment(hit.Sentiments, res.OriginalRequest.LowThreshold.Value, res.OriginalRequest.HighThreshold.Value);
                            foreach (var quarter in hit.Quarters.Values)
                            {
                                Sentiments s;
                                if (quarterSentiments.TryGetValue(quarter.StartPoint, out s))
                                {
                                    quarter.Sentiments = s;
                                }
                                else
                                {
                                    IQLogger.BuildDebug("No sentiment for quarter hit")
                                        .SetValue("IQCCKey", hit.Iqcckey)
                                        .Submit();

                                    // Set an empty sentiment object if no corresponding quarter sentiment was found
                                    quarter.Sentiments = new Sentiments();
                                }
                            }
                        }
                    }
                    ...
                    {
                        IQLogger.BuildDebug("No sentiment for hit")
                            .SetValue("IQCCKey", hit.Iqcckey)
                            .Submit();

                        // Add empty sentiment object to the hit
                        hit.Sentiments = new Sentiments();

                        // If quarter hits have been generated, add empty sentiment object to each quarter
                        if (res.OriginalRequest.GenerateQuarterHour && hit.Quarters != null && hit.Quarters.Count > 0)
                        {
                            foreach (var quarter in hit.Quarters.Values)
                            {
        ...
        private static void parseSentiment(XmlDocument doc, SearchTVResult res, string highlightFeildName)
        {
            CommonFunction.LogInfo("parseSentiment()");
            ...
            foreach (var _IQCCKeySentimentsPair in _IQCCKeyToSentimentsMap)
            {
                Hit _Hit = new Hit();
                _Hit.Iqcckey = _IQCCKeySentimentsPair.Key;
                _Hit.Sentiments = _IQCCKeySentimentsPair.Value;
                _Hits.Add(_Hit);
            }
            res.Hits = _Hits;
        }
                ...
                try
                {
                    ...
                    switch (cn.Attributes["name"].Value.ToLower(System.Globalization.CultureInfo.CurrentCulture))
                    {
                        ...
                        case "hour":
                            ...
                            break;
                        case "country_num":
                            hit.CountryNum = Convert.ToInt32(cn.InnerText, new System.Globalization.NumberFormatInfo());
                            break;
                        case "date":
        ...
        public SearchNewsResults SearchNews(SearchNewsRequest request, Int32? timeOutPeriod = null, Boolean isParagraphSearch = false)
        {
            ...
            try
            {
                IQLogger.Debug("IQSearch.SearchNews() - Begin");
                ...
                if (!string.IsNullOrWhiteSpace(request.SearchTerm))
                {
                    ...
                    if (request.IsSentiment || request.IsReturnHighlight || isParagraphSearch)
                    {
                        ...
                        if (request.FragSize.HasValue)
                        {
                            IQLogger.Debug("Frag size from request = " + request.FragSize.Value);
                            vars.Add(new KeyValuePair<string, string>("hl.fragsize", request.FragSize.Value.ToString()));
                        }
                        else if (request.IsSentiment)
                        {
                            IQLogger.Debug("Frag size for sentiment = " + ConfigurationManager.AppSettings["SolrFragSizeForSentiment"]);
                            vars.Add(new KeyValuePair<string, string>("hl.fragsize", ConfigurationManager.AppSettings["SolrFragSizeForSentiment"]));
                        }
                        else
                        {
                            IQLogger.Debug("Frag size from settings = " + ConfigurationManager.AppSettings["SolrFragSize"]);
                            vars.Add(new KeyValuePair<string, string>("hl.fragsize", ConfigurationManager.AppSettings["SolrFragSize"]));
                        }
                ...
                if (request.LastSequenceId.HasValue && request.LastSequenceId.Value > 0)
                {
                    ...
                    FQuery.AppendFormat("iqseqid:[{0} TO *]", request.LastSequenceId.Value + 1);
                }
                ...
                // so lets pass solr search url and all required params 
                // which will turn to request to solr and return solr response
                IQLogger.BuildDebug("IQSearch.SearchNews() - Create Solr Request")
                    .SetValue("q", q)
                    .SetValue("fq", fq)
                    .SetValue("outRequest", request.IsOutRequest)
                    .Submit();
                ...
                IQLogger.BuildDebug("IQSearch.SearchNews() - complete")
                    .SetValue("totalResults", res.TotalResults)
                    .SetValue("timeElapsed", sw.Elapsed)
                    .Submit();
        ...
        public SearchSMResult SearchSocialMedia(SearchSMRequest request, Int32? timeOutPeriod = null)
        {
            ...
            try
            {
                IQLogger.Debug("IQSearch.SearchSocialMedia() - Begin");
                ...
                // so lets pass solr search url and all required params 
                // which will turn to request to solr and return solr response
                IQLogger.BuildDebug("IQSearch.SearchSocialMedia() - Create Solr Out Request")
                    .SetValue("q", q)
                    .SetValue("fq", fq)
                    .SetValue("outRequest", request.IsOutRequest)
                    .Submit();
                ...
                IQLogger.BuildDebug("IQSearch.SearchNews() - Response Received")
                    .SetValue("timeElapsed", sw.Elapsed)
                    .Submit();
                ...
                IQLogger.BuildDebug("IQSearch.SearchNews() - Parsing Complete")
                    .SetValue("totalResults", res.TotalResults)
                    .SetValue("timeElapsed", sw.Elapsed)
                    .Submit();
                ...
                foreach (XElement elem in xDoc.Descendants("response").Descendants("result").Descendants("doc"))
                {
                    ...
                    foreach (XElement childelem in elem.Elements())
                    {
                        if (childelem.Attribute("name").Value == "iqseqid")
                        {
                            ...
                            if (res.OriginalRequest.IsReturnHighlight && !string.IsNullOrWhiteSpace(res.OriginalRequest.SearchTerm))
                            {
                                XElement highlightElem = (from c in highlightingEle.Descendants("lst")
                                                          where c.Attribute("name").Value == childelem.Value
                                                          select c).FirstOrDefault();
                                if (highlightElem != null)
                                {
                                    var highlightText = highlightElem.Descendants("str").FirstOrDefault();
                                    if (highlightText != null)
                                    {
                                        result.Highlights = new List<string>(1) { highlightText.Value };
                                    }
                                }
                            }
                        }
        ...
        public SearchSMResult SearchInstagram(SearchSMRequest request, Int32? timeOutPeriod = null)
        {
            ...
            try
            {
                ...
                // Tags/Users
                if (request.SocialMediaSources != null && request.SocialMediaSources.Count == 1 && request.SocialMediaSources[0] != null)
                {
